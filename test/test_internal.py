...
def test_path(monkeypatch):
    def isfile(fn):
        if fn in ('path1/foo.json.gz', 'path2/foo.json.gz'):
            return True
        if fn == 'pathmissing/foo.json.gz':
            return False
...
def test_path_with_prefixed_chem(monkeypatch):
    def isfile(fn):
        if fn in ('path1/SP2-C2.json.gz', 'path2/SP2-C2.json.gz', 'path1/SP3-C3.json.gz', 'path2/SP3-C3.json.gz'):
            return True
        if fn in ('pathmissing/SP2-C2.json.gz', 'pathmissing/SP3-C3.json.gz'):
            return False
        raise Exception('Called! {!r}'.format(fn))
    monkeypatch.setattr(os.path, 'isfile', isfile)
